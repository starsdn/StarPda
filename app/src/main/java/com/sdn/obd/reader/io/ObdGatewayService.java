...
import com.sdn.obd.commands.protocol.EchoOffObdCommand;
import com.sdn.obd.commands.protocol.LineFeedOffObdCommand;
import com.sdn.obd.commands.protocol.ObdResetCommand;
import com.sdn.obd.commands.protocol.SelectProtocolObdCommand;
import com.sdn.obd.commands.protocol.TimeoutObdCommand;
...
import com.sdn.obd.reader.IPostMonitor;
import com.star.obd.app.RequestAction;
import com.star.obd.app.ConfigActivity;
import com.star.R;
import com.sdn.obd.reader.io.ObdCommandJob.ObdCommandJobState;
...
public class ObdGatewayService extends Service {
	...
	private void startService() {
		...
		//remoteDevice = readString(this,"OBDMAC"); //得到保存的OBD蓝牙MAC
		
		if (remoteDevice == null || "".equals(remoteDevice)) {
			Toast.makeText(this, "没有可以蓝牙OBD设备",Toast.LENGTH_LONG).show();
			Log.e(TAG, "");
			// TODO 完美的结束服务
			stopService();
		}
		
		_dev = btAdapter.getRemoteDevice(remoteDevice); //得到对应的蓝牙设备
	 ...
	 * @return 存储内容，如果没有则返回""
	 */
	public  String readString(Context context,String name){
		SharedPreferences sp = context.getSharedPreferences("sdn", Context.MODE_PRIVATE);
	...
	private void showNotification() {
		...
		PendingIntent contentIntent = PendingIntent.getActivity(this, 0,
		        new Intent(this, RequestAction.class), 0);

		// 设置在通知面板中显示的视图的信息。
		//context，标题，内容，和pendingIntnet
